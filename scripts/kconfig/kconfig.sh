...
KCONFIG_FILE="" DOT_CONFIG="" MANIFEST_FILE="" CONFIG_SH_OUT=""
...
require() {
  for var in "$@"; do
    [[ -n "${!var}" ]] || error 1 "$var is required for this command (pass --${var//_/-} flag)"
  done
}
...
case "$CMD" in
  menuconfig|guiconfig)
    ...
    gen_config "$DOT_CONFIG" "$CONFIG_SH_OUT"
    ;;
  ...
  gen-config)
    require DOT_CONFIG CONFIG_SH_OUT
    gen_config "$DOT_CONFIG" "$CONFIG_SH_OUT"
    ;;
